...
if __name__ == '__main__':
    mon = monitors.Monitor(
        ...
        width=40, distance=60,  # width 显示器尺寸cm; distance 受试者与显示器间的距离
        verbose=False
    )
    ...
    AVEP
    '''
    n_elements, rows, columns = 32, 4, 8  # n_elements 指令数量;  rows 行;  columns 列
    ...
    stim_time = 1  # 刺激时长
    stim_opacities = 1  # 刺激对比度
    freqs = np.ones((n_elements)) * 10  # 指令的频率
    # phases = np.array([i * 0.35 % 2 for i in range(n_elements)])  # 指令的相位
    stim_num = 2
    avep = AVEP(win=win, dot_shape='cluster')
    sequence = [avep.num2bin_ary(i, 5) for i in range(n_elements)]
    ...
    lsl_source_id = 'meta_online_worker'  # None                 # source id
    online = False  # True                                       # 在线实验的标志
    ex.register_paradigm('continous SSVEP', paradigm, VSObject=basic_ssvep, bg_color=bg_color,
